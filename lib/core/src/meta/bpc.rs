...
async fn update_bpc(duration_between_measures: u64, interface: AmInterface, bpc_state: Arc<Mutex<BpcInterface>>)
-> Result<(), PlatformError>
{
    ...
    loop {
        let enable_value = match bpc_state.lock().await
            ...
            Ok(v) => v,
            Err(e) => {
                interface.lock().await.set_event_error("Bpc has been disconnected");
                return __platform_error_result!(format!("bpc has been disconnected : {:?}", e));
            }
        };
...
impl interface::fsm::States for BpcStates {
    ...
    -> Result<(), PlatformError>
    {
        ...
        tokio::spawn(update_bpc(10000, interface_cloned, bpc_interface));
