...
/// reboot interface
/// 
async fn update_blc(duration_between_measures: u64, interface: AmInterface, blc_state: Arc<Mutex<BlcInterface>>, attributes_used: Vec<String>, device_name: String)
-> Result<(), PlatformError>
{
    ...
    loop {
        ...
        // correctly)
        if attributes_used.contains(&BlcAttributes::AnalogModulation.to_string()) {

            // Init analog modulation (should disable analog modulation)
            let analog_modulation = match blc_state.lock().await
        ...
        if attributes_used.contains(&BlcAttributes::Mode.to_string()) {

            // Init mode
            let mode_value = match blc_state.lock().await
        ...
        if attributes_used.contains(&BlcAttributes::Enable.to_string()) {

            // Resend enable attribute fields values
            let enable_value = match blc_state.lock().await
        ...
        if attributes_used.contains(&BlcAttributes::Power.to_string()) {
            let power_attribute_str = BlcAttributes::Power.as_str();

            // Init power
            let max_power = match blc_state.lock().await
                .actions.read_power_max(&interface).await {
                Ok(v) => v,
                Err(e) => {
                    interface.lock().await.set_event_error(format!("{:?} has been disconnected", device_name));
                    return __platform_error_result!(format!("{:?} has been disconnected : {:?}", device_name, e));
            ...
            let power_value = match blc_state.lock().await
            ...
            interface.lock().await.update_attribute_with_f64(power_attribute_str, "min", blc_state.lock().await.params.power_min );
            ...
            interface.lock().await.update_attribute_with_f64(power_attribute_str, "decimals", blc_state.lock().await.params.power_decimals as f64);
        ...
        if attributes_used.contains(&BlcAttributes::Current.to_string()) { 
            let current_str = BlcAttributes::Current.as_str();

            interface.lock().await.register_attribute(JsonAttribute::new_boxed(current_str, true));

            // Resend current attribute fields values

            let current_max = match blc_state.lock().await
                .actions.read_max_current_value(&interface).await {
                Ok(v) => v,
                Err(e) => {
                    interface.lock().await.set_event_error(format!("{:?} has been disconnected", device_name));
                    return __platform_error_result!(format!("{:?} has been disconnected : {:?}", device_name, e));
            ...
            let current_value = match blc_state.lock().await
            ...
            interface.lock().await.update_attribute_with_f64(current_str, "min", blc_state.lock().await.params.current_min );
            interface.lock().await.update_attribute_with_f64(current_str, "max", current_max );
            interface.lock().await.update_attribute_with_f64(current_str, "value", current_value);
            interface.lock().await.update_attribute_with_f64(current_str, "decimals", blc_state.lock().await.params.current_decimals as f64);
...
impl interface::fsm::States for BlcStates {
    ...
    -> Result<(), PlatformError>
    {
        ...
        let blc_interface = Arc::clone(&(self.blc_interface));
        let interface_cloned = Arc::clone(&interface);
        let device_name = interface.lock().await._dev_name().clone();

        tokio::spawn(update_blc(10000, interface_cloned, blc_interface, self.attributes_used.clone(), device_name));
