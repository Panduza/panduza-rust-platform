...
use panduza_connectors::usb::usb2::get as get_connector;
use panduza_connectors::usb::usb2::Connector as UsbConnector;
...
struct LBX488BlcActions {
    connector_usb: Option<UsbConnector>,
...
impl LBX488BlcActions {
    ...
    async fn ask_float(&mut self, command: &[u8]) -> Result<f64, PlatformError> {
        let f = self.ask(command)
            .await?
            .trim_end_matches("\0")
            .to_string()
            .parse::<f64>()
            .map_err(|e| platform_error!(
                "Unexpected answer from Oxxius LBX488 : could not parse as float, error message {:?}",
                e
            ))?;

        // Use the decimal library to have a better precision
        let value_dec = Decimal::from_f64(f).ok_or(platform_error!(
            "Unexpected answer form Oxxius LBX488 : could not parse as Decimal"
        ))?;

        let final_value = (value_dec * dec!(0.001)).to_f64().ok_or(platform_error!(
            "Unexpected answer form Oxxius LBX488 : could not parse as f64"
        ))?;
...
impl blc::BlcActions for LBX488BlcActions {
    ...
    async fn initializating(&mut self, interface: &AmInterface) -> Result<(), PlatformError> {
        self.connector_usb = Some(get_connector(&self.serial_config).await?);
        self.connector_usb
            .as_ref()
            .ok_or(platform_error!("Connector is not initialized"))?
            .lock()
        ...
        interface
            .lock()
            .await
            .log_info(format!("LBX_488 - initializing: {}", result));
    ...
    ///
    async fn read_analog_modulation(
        &mut self,
        ...
        interface.lock().await.log_info(format!(
            "read analog modulation value : {}",
            self.analog_modulation
        ));
    ...
    ///
    async fn write_analog_modulation(
        &mut self,
        interface: &AmInterface,
        v: bool,
        ...
        interface
            .lock()
            .await
            .log_info(format!("write analog modulation value : {}", v));
    ...
    async fn write_mode_value(
        ...
        interface
            .lock()
            .await
            .log_info(format!("write mode value : {}", v));
    ...
    async fn write_enable_value(
        ...
        interface
            .lock()
            .await
            .log_info(format!("write enable value : {}", status));
        return Ok(());
    }
    ...
    async fn read_power_max(&mut self, interface: &AmInterface) -> Result<f64, PlatformError> {
        ...
        interface
            .lock()
            .await
            .log_info(format!("read max power : {}", self.power_max));
    ...
    async fn read_power_value(&mut self, interface: &AmInterface) -> Result<f64, PlatformError> {
        ...
        interface
            .lock()
            .await
            .log_info(format!("read power : {}", self.power_value));
    ...
    ///
    async fn write_power_value(
        &mut self,
        interface: &AmInterface,
        v: f64,
    ) -> Result<(), PlatformError> {
        let value_dec = Decimal::from_f64(v).ok_or(platform_error!(
            "Unexpected answer form Oxxius LBX488 : could not parse as Decimal"
        ))?;
        ...
        let val_f64 = val_mw.to_f64().ok_or(platform_error!(
            "Unexpected answer form Oxxius LBX488 : could not parse as f64"
        ))?;
        ...
        interface
            .lock()
            .await
            .log_info(format!("write power : {}", command));
    ...
    async fn read_max_current_value(
        &mut self,
        interface: &AmInterface,
        ...
        let value_max = value_max_dec.to_f64().ok_or(platform_error!(
            "Unexpected answer form Oxxius LBX488 : could not parse as f64"
        ))?;
        ...
        interface
            .lock()
            .await
            .log_info(format!("read max current : {}", self.current_value));
    ...
    async fn read_current_value(&mut self, interface: &AmInterface) -> Result<f64, PlatformError> {
        ...
        interface
            .lock()
            .await
            .log_info(format!("read current : {}", self.current_value));
    ...
    async fn write_current_value(
        ...
        let val_f64 = val_ma.to_f64().ok_or(platform_error!(
            "Unexpected answer form Oxxius LBX488 : could not parse as f64"
        ))?;
        ...
        interface
            .lock()
            .await
            .log_info(format!("write current : {}", val_ma));
...
/// Interface
///
pub fn build<A: Into<String>>(name: A, serial_config: &UsbConfig) -> InterfaceBuilder {
    return blc::build(
        name,
        blc::BlcParams {
            power_min: 0.0,
